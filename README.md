...
**Локальный мультимодальный поиск по личной коллекции фотографий** — без облака; пользовательские фото не покидают машину (веса моделей при первом запуске могут скачаться из публичных репозиториев).

I-Mage индексирует каталог изображений на компьютере и позволяет искать фотографии по смыслу, объектам и лицам. В основе — три ML-модуля, векторный поиск, инкрементальная индексация, фоновая обработка и Web UI.
...
| «Каталог вырос — не хочу пересканировать всё заново» | Инкрементальная индексация по content hash и module status |
...
```text
                    ┌─────────────────────────────────────────┐
  Запрос пользователя│  текст / фото лица / класс объекта     │
                    └─────────────────┬───────────────────────┘
                                      │
           ┌───────────────────────────┼───────────────────────────┐
           ▼                           ▼                           ▼
    ┌─────────────┐            ┌─────────────┐            ┌─────────────┐
    │    CLIP     │            │   YOLOv8    │            │  ArcFace    │
    │ ViT-B/32    │            │   (COCO)    │            │ (InsightFace│
    │             │            │             │            │  buffalo_l) │
    └──────┬──────┘            └──────┬──────┘            └──────┬──────┘
           │ эмбеддинг 512d           │ bbox + label             │ эмбеддинг 512d
           ▼                           ▼                           ▼
    ┌─────────────┐            ┌─────────────┐            ┌─────────────┐
